...
def _learning_curve_test_data_set(classifiers, dataset_id, dataset_getter,
    ...
    for classifier_id, classifier_class in classifiers.iteritems():
        ...
        for test_folds in train_sets:
            if verbose:
                print >> stderr, 'Sample size: ({0}/{1})'.format(
                        len(test_folds[0]), len(train))

            scores = []
            for test_fold_i, test_fold in enumerate(test_folds, start=1):
                #if verbose:
                #    print >> stderr, 'Test fold: ({0}/{1})'.format(
                #            test_fold_i, len(test_folds))
                ...
                #if verbose:
                #    print >> stderr, 'Evaluating...',
                scores.append(_score_classifier(classifier, dev))
                #if verbose:
                #    print >> stderr, 'Done!'
